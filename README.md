...
- `app.R` — Shiny app comparing GTFS and TDM transit layers on a Carto Positron/Dark Matter basemap, either **overlaid** on one map or **swiped** side by side (`mapgl::compare()`, toggled via a switch in the navbar). All settings live in a configuration modal (gear icon, top-right) rather than a permanent sidebar: GTFS source (a saved snapshot, an uploaded GTFS zip, or a live feed URL), TDM year (base year 2023 vs. forecast scenarios) and line type (rail/BRT/core), and what each side shows (lines/stops/both).
- `R/gtfs_pipeline.R` — the shared GTFS extraction pipeline (flatten zip layout → `tidytransit::read_gtfs()` → shape-level routes + stops with GTFSx-style colors). `app.R` calls it live for every GTFS source — a saved snapshot read straight from `_data/gtfs/raw/`, an uploaded zip, or a downloaded feed URL — there's no pre-baked/cached copy to keep in sync; a snapshot is processed fresh each time it's selected, the same as an upload would be.
...
- `_data/gtfs/raw/` — GTFS snapshots (original, unmodified zip downloads), one per feed publication date. This is the only GTFS data committed to the repo — no derived/processed copies.
...
1. Download/refresh a GTFS feed into `_data/gtfs/raw/` — no separate processing step needed, the app reads raw zips directly and reprocesses on demand.
2. Update `_data/tdm/WFv1000_MasterNet_20260430.gdb.zip` with the latest model export, then run `Rscript R/prepare_tdm.R`.
3. Run the app (`shiny::runApp()`). On launch it opens a configuration modal (reopen anytime via the gear icon): pick a GTFS source (saved snapshot / upload / feed URL), TDM year and line type, and what each side shows. Pick Overlay or Swipe comparison mode via the navbar switch, then visually compare route alignment and stop coverage between the two datasets.
